...
static int nuc_to_internal(char c);
...
float get_pst_prob(struct pst_node* n, char* string,int target, int pos)
{
        ...
        if(pos == 0){
                ...
                for(c = 0;c < 5;c++){
                        if(n->nuc_probability[c] >= p){
                                p = n->nuc_probability[c];
                        }
                }
                //LOG_MSG("Found %f %f ", p , n->nuc_probability[target]);
        ...
        if(len + 1 < MAX_PST_LEN ){
                for(i = 0; i < 5;i++){
                        ...
                        if(n->nuc_probability[i] >= pst->p_min){ /// string (suffix  + letter is not rare compared to all other letters  ) // e.g. acgC = 0.2 and not acgC = 0.0000001. hence it makes sense to "extend the suffix - test [A,C,G,T] - acgC
                                ...
                                /*add = 0;
                                for(j = 0; j < 5;j++){
                                        if(tmp_counts_s[j] /  (pst->numseq * (pst->mean_length - (len+1))) > 0.00000001){
                                                add = 1;
                                                break;
                                        }
                                }
                                ...
                                if(sum){
                                        ...
                                        if(add){
                                                ...
                                                for(j = 0; j < 5;j++){
                                                        ...
                                                        n->next[i]->nuc_probability[j] = tmp_counts_s[j] *(1.0f  - 5.0f *  pst->gamma_min) + pst->gamma_min;
