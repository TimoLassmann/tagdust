        ...
        for(i = 0; i < m->num_hmms;i++){
                ...
                for(j = 0; j < len;j++){
                        ...
                        if(current_nuc < 4){
                                ...
                                ERROR_MSG("No idea what this could be: %c", tmp[j]);
                        }
                }
        }
        return OK;
...
struct model* malloc_model_according_to_read_structure(int num_hmm, int length,int dyn_length)
{
        ...
        for(i = 0; i < model->num_hmms;i++){
                ...
                for(j = 0; j < len;j++){
                        ...
                        model->hmms[i]->hmm_column[j] = NULL;
                        ...
                        col->M_foward = 0;
                        for(c = 0; c < 9;c++){
                                col->transition[c] = prob2scaledprob(0.0f);
                                col->transition_e[c] = prob2scaledprob(0.0f);
                        }
                        for(c = 0; c < 5;c++){
                                col->m_emit[c] = prob2scaledprob(0.0f);
                                col->m_emit_e[c] = prob2scaledprob(0.0f);
                                col->i_emit[c] = prob2scaledprob(0.0f);
                                col->i_emit_e[c] = prob2scaledprob(0.0f);
                        }

                        MMALLOC(col->M_foward,sizeof(float) * (dyn_length+1));
                        MMALLOC(col->M_backward ,sizeof(float) * (dyn_length+1));
                        MMALLOC(col->I_foward,sizeof(float) * (dyn_length+1));
                        MMALLOC(col->I_backward,sizeof(float) * (dyn_length+1));
                        MMALLOC(col->D_foward,sizeof(float) * (dyn_length+1));
                        MMALLOC(col->D_backward, sizeof(float) * (dyn_length+1));
                }
        }
        return model;
ERROR:
