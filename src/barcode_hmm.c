...
void hmm_controller(struct parameters* param,int (*fp)(struct read_info** ,struct parameters*,FILE* ),int file_num)
{
	...
	for(i = 0; i < param->num_query /2 ;i++){
	
		ri[i] = emit_read_sequence(mb,ri[i] , average_length,&seed);
	
	}
	for(i = param->num_query/2; i < param->num_query;i++){
	...
	for(i = 0; i < param->num_query ;i++){
		//if(ri[i]->name[0] == 'P'){
		/*	fprintf(stdout,"%f	%d	%s\n", ri[i]->prob,(int) floor(ri[i]->prob* 10000.0  + 0.5),ri[i]->name);
		//}
		if(ri[i]->name[0] == 'P'){
			if(ri[i]->prob  < 0.01){
				fprintf(stdout,"%s	%f	%d\n",ri[i]->name,ri[i]->prob ,ri[i]->len);
				for(j = 0;j < ri[i]->len;j++){
					fprintf(stdout,"%d",ri[i]->seq[j]);
				}
				fprintf(stdout,"\n");
			}
		}
		*/
		if(ri[i]->name[0] == 'P'){
			P[(int) floor(ri[i]->prob* 10000.0  + 0.5)]++;
		}
		
		if(ri[i]->name[0] == 'N'){
			N[(int) floor(ri[i]->prob* 10000.0  + 0.5)]++;
		}
	...
	for(i = 9999; i >= 0 ;i--){
		...
		sensitivity = P[i] /  (float)(param->num_query/2);
		specificity = ((float)( param->num_query/2) - N[i] )/ (float)(param->num_query/2);
		
		//fprintf(stdout,"%f	%f	%f	%f\n", (float)i / 10000.0, sensitivity,specificity,sensitivity + specificity);
		if(sensitivity + 10.0*specificity > max){
			max = sensitivity + 10.0*specificity;
			cut = (float)i / 10000.0;
		}
		
		if(N[i] / (P[i] + N[i]) < 0.01 ){
			cut2 = (float)i / 10000.0;
		}
	...
	while ((numseq = fp(ri, param,file)) != 0){
		...
		for(i = 0; i < numseq;i++){
			fprintf(stdout,"%e\n",(float) N[(int) floor(ri[i]->prob* 10000.0  + 0.5)] / (float)(param->num_query /2));
 		}
...
//#if DEBUG
		...
		for(i = 0 ; i < model->num_hmms;i++){
			...
			for(j = 0; j < len;j++){
				...
				col->transition[MD] = prob2scaledprob(base_error * indel_freq*0.5);
				col->transition[MSKIP] = prob2scaledprob(0.00f);
				
				col->transition[II] = prob2scaledprob(1.0 - 0.999);
				col->transition[IM] = prob2scaledprob(0.999);
				col->transition[ISKIP] = prob2scaledprob(0.00f);
	...
	for(c = 1; c < mb->num_models-1;c++){
		if(param->read_structure->type[c] == 'P'){
			...
			for(i = 0 ; i < model->num_hmms;i++){
				...
				col = model->hmms[i]->hmm_column[j];
				model->silent_to_M[i][j]  = prob2scaledprob(1.0 / (float) model->num_hmms) + prob2scaledprob(0.9);
				for(j = 1; j < len;j++){
					model->silent_to_M[i][j]  = prob2scaledprob(1.0 / (float) model->num_hmms) + prob2scaledprob(0.1 / (float)(len-1));
				}
				//model->silent_to_M[i][len-1] = prob2scaledprob(0.0);
				//model->M_to_silent[i] = prob2scaledprob(1.0);
				
				for(j = 0; j < len;j++){
					col = model->hmms[i]->hmm_column[j];
					
					col->transition[MSKIP] = prob2scaledprob(0.1 / (float)(len-1));
					...
					col->transition[ISKIP] = prob2scaledprob(0.1 / (float)(len-1));
					
					col->transition[II] = prob2scaledprob(1.0 - 0.999)+ prob2scaledprob (1.0 - scaledprob2prob(col->transition[ISKIP]  ));
					col->transition[IM] = prob2scaledprob(0.999)+ prob2scaledprob (1.0 - scaledprob2prob(col->transition[ISKIP]  ));
			...
			/*for(j = 0; j < len;j++){
				...
				fprintf(stderr,"%f	%f	%f	%f	%f	%d len \n",scaledprob2prob(model->silent_to_M[0][j]),scaledprob2prob( col->transition[MM] ), scaledprob2prob(col->transition[MI] ), scaledprob2prob(col->transition[MD]), scaledprob2prob(col->transition[MSKIP]),j);
			}*/
		}
	}
...
//#if DEBUG
		...
		for(i = 0 ; i < model->num_hmms;i++){
			...
			for(j = 0; j < len;j++){
				...
				col->transition[ISKIP] = prob2scaledprob(   gaussian_pdf(j+1 , mean ,stdev) / sum_prob)  - model->silent_to_M[i][0];
	...
	for(g = 0;g < mb->num_models;g++){
		...
		for(i = 0; i < m->num_hmms;i++){
			...
			for(j = 0; j < m->hmms[i]->num_columns;j++){
				...
				//copy->silent_to_I[i][j] = org->silent_to_I[i][j];
				m->silent_to_I_e[i][j] = 1.0f; //org->silent_to_I_e[i][j];
				//copy->silent_to_M[i][j] = org->silent_to_M[i][j];
				m->silent_to_M_e[i][j] = 1.0f; //org->silent_to_M_e[i][j];
				
				for(c = 0; c< 5;c++){
					//copy_col->i_emit[c] = org_col->i_emit[c];
					hmm_column->i_emit_e[c] = 1.0f;
					
					//copy_col->m_emit[c] = org_col->m_emit[c];
					hmm_column->m_emit_e[c] = 1.0f;
				}
				
				for(c = 0; c < 9;c++){
					//c/opy_col->transition[c] = org_col->transition[c];
					hmm_column->transition_e[c] = 1.0f;
				}
			}
		}
		//copy->skip = org->skip;
		m->skip_e = 1.0f;//?//?org->skip_e;
		//copy->num_hmms = org->num_hmms;
	}
	return mb;
}
